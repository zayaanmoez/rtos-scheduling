...
#define SCHED_NAME "Modulo Round Robin Scheduler"


void schedule_task_morr(tcb_t **ready_queue, tcb_t **running_task, Interval **schedule,
		...
		do {
			for(int i = 0; i < num_tasks; i++) {
				if(task_list[i]->state == STATE_WAITING) {
					...
					ready_task(i);
					insertQueue(ready_queue, task_list[i], &numReady);
					moduloSort(ready_queue, numReady);
