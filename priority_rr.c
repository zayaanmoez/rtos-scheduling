...
#define SCHED_NAME "Priority Based Round Robin Scheduler"


void schedule_task_pbrr(tcb_t **ready_queue, tcb_t **running_task, Interval **schedule,
		int *numReady, int currentTime, int quantum) {
	if(*running_task != NULL) {
		...
		if((*running_task)->params.runTime == (*running_task)->params.burstTime) {
			...
			*running_task = NULL;
		} else if(*numReady > 0 && (*running_task)->params.runTime%quantum == 0) {
